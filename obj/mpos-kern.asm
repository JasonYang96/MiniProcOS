...
schedule(void)
{
  ...
  10008f:	a1 78 9f 10 00       	mov    0x109f78,%eax
	while (1) {
		pid = (pid + 1) % NPROCS;
...
schedule(void)
{
  ...
  1000a4:	83 b8 18 92 10 00 01 	cmpl   $0x1,0x109218(%eax)
  1000ab:	75 ee                	jne    10009b <schedule+0xf>
			run(&proc_array[pid]);
  1000ad:	83 ec 0c             	sub    $0xc,%esp
  1000b0:	05 d0 91 10 00       	add    $0x1091d0,%eax
  1000b5:	50                   	push   %eax
...
interrupt(registers_t *reg)
{
  ...
  1000c4:	83 ec 1c             	sub    $0x1c,%esp
	...
	// registers into the 'current' process descriptor.
	current->p_registers = *reg;
  1000c7:	8b 1d 78 9f 10 00    	mov    0x109f78,%ebx
...
interrupt(registers_t *reg)
{
  ...
  1000e1:	0f 87 31 01 00 00    	ja     100218 <interrupt+0x15d>
  ...
  1000fc:	b8 6c 92 10 00       	mov    $0x10926c,%eax
  ...
  100116:	8d 90 d0 91 10 00    	lea    0x1091d0(%eax),%edx
  ...
  100123:	8b b0 d0 91 10 00    	mov    0x1091d0(%eax),%esi
	for (i = 1; i < NPROCS ; i++)
	{
		//empty process found
		if (proc_array[i].p_state == P_EMPTY)
		{
			proc_array[i].p_state = P_RUNNABLE;
  100129:	c7 80 18 92 10 00 01 	movl   $0x1,0x109218(%eax)
  100130:	00 00 00 
  ...
  10013f:	c7 80 f0 91 10 00 00 	movl   $0x0,0x1091f0(%eax)
  100146:	00 00 00 
	// We have done one for you.
	src_stack_top = PROC1_STACK_ADDR + src->p_pid * PROC_STACK_SIZE;
	src_stack_bottom = src->p_registers.reg_esp;
  ...
  100158:	e8 b7 03 00 00       	call   100514 <memcpy>
  ...
  10017b:	ff 35 78 9f 10 00    	pushl  0x109f78
  100181:	e8 b6 02 00 00       	call   10043c <run>
	case INT_SYS_YIELD:
  ...
  10018b:	a1 78 9f 10 00       	mov    0x109f78,%eax
  ...
  1001a7:	c7 40 50 00 00 00 00 	movl   $0x0,0x50(%eax)
		...
		{
			proc_array[current->p_wait_pid].p_state = P_RUNNABLE;
  1001ae:	c7 82 18 92 10 00 01 	movl   $0x1,0x109218(%edx)
  1001b5:	00 00 00 
			proc_array[current->p_wait_pid].p_registers.reg_eax = current->p_exit_status;
  1001b8:	89 8a f0 91 10 00    	mov    %ecx,0x1091f0(%edx)
			current->p_wait_pid = 0;
		}
		schedule();
  1001be:	e8 c9 fe ff ff       	call   10008c <schedule>
  ...
  1001c3:	a1 78 9f 10 00       	mov    0x109f78,%eax
  1001c8:	8b 50 20             	mov    0x20(%eax),%edx
		if (p <= 0 || p >= NPROCS || p == current->p_pid
  1001cb:	8d 4a ff             	lea    -0x1(%edx),%ecx
  1001ce:	83 f9 0e             	cmp    $0xe,%ecx
  1001d1:	77 11                	ja     1001e4 <interrupt+0x129>
  1001d3:	3b 10                	cmp    (%eax),%edx
  1001d5:	74 0d                	je     1001e4 <interrupt+0x129>
		    || proc_array[p].p_state == P_EMPTY)
  1001d7:	6b d2 54             	imul   $0x54,%edx,%edx
  1001da:	8b 8a 18 92 10 00    	mov    0x109218(%edx),%ecx
		// (In the Unix operating system, only process P's parent
		// can call sys_wait(P).  In MiniprocOS, we allow ANY
		// process to call sys_wait(P).)
		...
		pid_t p = current->p_registers.reg_eax;
		if (p <= 0 || p >= NPROCS || p == current->p_pid
  1001e0:	85 c9                	test   %ecx,%ecx
  1001e2:	75 09                	jne    1001ed <interrupt+0x132>
		    || proc_array[p].p_state == P_EMPTY)
			current->p_registers.reg_eax = -1;
  1001e4:	c7 40 20 ff ff ff ff 	movl   $0xffffffff,0x20(%eax)
		// (In the Unix operating system, only process P's parent
		// can call sys_wait(P).  In MiniprocOS, we allow ANY
		// process to call sys_wait(P).)
		...
		pid_t p = current->p_registers.reg_eax;
		if (p <= 0 || p >= NPROCS || p == current->p_pid
  1001eb:	eb 26                	jmp    100213 <interrupt+0x158>
		    || proc_array[p].p_state == P_EMPTY)
			current->p_registers.reg_eax = -1;
		else if (proc_array[p].p_state == P_ZOMBIE)
  1001ed:	83 f9 03             	cmp    $0x3,%ecx
  1001f0:	75 0b                	jne    1001fd <interrupt+0x142>
			current->p_registers.reg_eax = proc_array[p].p_exit_status;
  1001f2:	8b 92 1c 92 10 00    	mov    0x10921c(%edx),%edx
  1001f8:	89 50 20             	mov    %edx,0x20(%eax)
  1001fb:	eb 16                	jmp    100213 <interrupt+0x158>
		else
		{
			current->p_registers.reg_eax = WAIT_TRYAGAIN;
  1001fd:	c7 40 20 fe ff ff ff 	movl   $0xfffffffe,0x20(%eax)
			current->p_state = P_BLOCKED;
  100204:	c7 40 48 02 00 00 00 	movl   $0x2,0x48(%eax)
			proc_array[p].p_wait_pid = current->p_pid ;
  10020b:	8b 00                	mov    (%eax),%eax
  10020d:	89 82 20 92 10 00    	mov    %eax,0x109220(%edx)
		}
		schedule();
  100213:	e8 74 fe ff ff       	call   10008c <schedule>
  100218:	eb fe                	jmp    100218 <interrupt+0x15d>
...
start(void)
{
  ...
  10021e:	68 40 05 00 00       	push   $0x540
  100223:	6a 00                	push   $0x0
  100225:	68 d0 91 10 00       	push   $0x1091d0
  10022a:	e8 49 03 00 00       	call   100578 <memset>
  10022f:	b8 d0 91 10 00       	mov    $0x1091d0,%eax
  100234:	31 d2                	xor    %edx,%edx
  ...
  10023c:	c7 40 48 00 00 00 00 	movl   $0x0,0x48(%eax)
		proc_array[i].p_wait_pid = 0;
  100243:	c7 40 50 00 00 00 00 	movl   $0x0,0x50(%eax)
	int whichprocess;
	pid_t i;
	...
	memset(proc_array, 0, sizeof(proc_array));
	for (i = 0; i < NPROCS; i++) {
  10024a:	83 c0 54             	add    $0x54,%eax
  10024d:	83 fa 10             	cmp    $0x10,%edx
  100250:	75 e7                	jne    100239 <start+0x1f>
		proc_array[i].p_state = P_EMPTY;
		proc_array[i].p_wait_pid = 0;
	}
  ...
  100252:	c7 05 78 9f 10 00 24 	movl   $0x109224,0x109f78
  100259:	92 10 00 
  ...
  100264:	ff 35 78 9f 10 00    	pushl  0x109f78
  ...
  10029f:	77 f3                	ja     100294 <start+0x7a>
	console_clear();
  ...
  1002a6:	a1 78 9f 10 00       	mov    0x109f78,%eax
  1002ab:	83 c0 34             	add    $0x34,%eax
  1002ae:	52                   	push   %edx
  1002af:	52                   	push   %edx
  1002b0:	50                   	push   %eax
  1002b1:	53                   	push   %ebx
  ...
  1002b7:	a1 78 9f 10 00       	mov    0x109f78,%eax
...
segments_init(void)
{
  ...
  1002d4:	b8 10 97 10 00       	mov    $0x109710,%eax
...
segments_init(void)
{
  ...
  1002f5:	88 15 a8 1a 10 00    	mov    %dl,0x101aa8
	...
	for (i = 0; i < sizeof(interrupt_descriptors) / sizeof(gatedescriptor_t); i++)
		SETGATE(interrupt_descriptors[i], 0,
  1002fb:	ba 78 97 10 00       	mov    $0x109778,%edx
segments_init(void)
{
	int i;
...
segments_init(void)
{
  ...
  10031e:	c7 05 14 97 10 00 00 	movl   $0x80000,0x109714
  100325:	00 08 00 
	kernel_task_descriptor.ts_ss0 = SEGSEL_KERN_DATA;
  100328:	66 c7 05 18 97 10 00 	movw   $0x10,0x109718
  10032f:	10 00 
	...
	for (i = 0; i < sizeof(interrupt_descriptors) / sizeof(gatedescriptor_t); i++)
		SETGATE(interrupt_descriptors[i], 0,
  100331:	66 89 0c c5 78 97 10 	mov    %cx,0x109778(,%eax,8)
  100338:	00 
...
run(process_t *proc)
{
  10043c:	8b 44 24 04          	mov    0x4(%esp),%eax
	current = proc;
  100440:	a3 78 9f 10 00       	mov    %eax,0x109f78
