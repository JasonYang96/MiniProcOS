...
multiboot_start:
  ...
  100013:	9d                   	popf   
	call start
  100014:	e8 12 02 00 00       	call   10022b <start>
  100019:	90                   	nop
...
schedule(void)
{
  ...
  10008f:	a1 30 9f 10 00       	mov    0x109f30,%eax
	while (1) {
		pid = (pid + 1) % NPROCS;
...
schedule(void)
{
  ...
  1000a4:	83 b8 d0 91 10 00 01 	cmpl   $0x1,0x1091d0(%eax)
  1000ab:	75 ee                	jne    10009b <schedule+0xf>
			run(&proc_array[pid]);
  1000ad:	83 ec 0c             	sub    $0xc,%esp
  1000b0:	05 88 91 10 00       	add    $0x109188,%eax
  1000b5:	50                   	push   %eax
...
interrupt(registers_t *reg)
{
  ...
  1000c4:	83 ec 1c             	sub    $0x1c,%esp
	...
	// registers into the 'current' process descriptor.
	current->p_registers = *reg;
  1000c7:	8b 1d 30 9f 10 00    	mov    0x109f30,%ebx
...
interrupt(registers_t *reg)
{
  ...
  1000e1:	0f 87 42 01 00 00    	ja     100229 <interrupt+0x16e>
  ...
  1000f3:	89 43 20             	mov    %eax,0x20(%ebx)
		run(current);
  1000f6:	53                   	push   %ebx
  1000f7:	e9 85 00 00 00       	jmp    100181 <interrupt+0xc6>
  1000fc:	b8 24 92 10 00       	mov    $0x109224,%eax
  ...
  100123:	8b b0 88 91 10 00    	mov    0x109188(%eax),%esi
	for (i = 1; i < NPROCS ; i++)
	{
		...
		{
			proc_array[i].p_state = P_RUNNABLE;
  100129:	c7 80 d0 91 10 00 01 	movl   $0x1,0x1091d0(%eax)
  100130:	00 00 00 
	...
	src_stack_top = PROC1_STACK_ADDR + src->p_pid * PROC_STACK_SIZE;
	src_stack_bottom = src->p_registers.reg_esp;
  100133:	8b 7b 40             	mov    0x40(%ebx),%edi
	uint32_t difference = src_stack_top - src_stack_bottom;
  100136:	83 c1 0a             	add    $0xa,%ecx
  100139:	c1 e1 12             	shl    $0x12,%ecx
	dest_stack_top = PROC1_STACK_ADDR + dest->p_pid * PROC_STACK_SIZE;
	dest_stack_bottom = dest_stack_top - difference;
  ...
  10013f:	c7 80 a8 91 10 00 00 	movl   $0x0,0x1091a8(%eax)
  100146:	00 00 00 
	// We have done one for you.
	src_stack_top = PROC1_STACK_ADDR + src->p_pid * PROC_STACK_SIZE;
	src_stack_bottom = src->p_registers.reg_esp;
  ...
  10014f:	51                   	push   %ecx
	...
	dest_stack_top = PROC1_STACK_ADDR + dest->p_pid * PROC_STACK_SIZE;
	dest_stack_bottom = dest_stack_top - difference;
  100150:	29 ce                	sub    %ecx,%esi
	//memcpy the stack and set dest->p_registers.reg_esp
	memcpy((void *)dest_stack_bottom, (void *)src_stack_bottom, difference);
  100152:	57                   	push   %edi
  100153:	56                   	push   %esi
  100154:	89 54 24 1c          	mov    %edx,0x1c(%esp)
  100158:	e8 c7 03 00 00       	call   100524 <memcpy>
	dest->p_registers.reg_esp = dest_stack_bottom;
  10015d:	8b 54 24 1c          	mov    0x1c(%esp),%edx
  100161:	83 c4 10             	add    $0x10,%esp
  100164:	89 72 40             	mov    %esi,0x40(%edx)
  100167:	eb 0c                	jmp    100175 <interrupt+0xba>
	...
	for (i = 1; i < NPROCS ; i++)
  100169:	45                   	inc    %ebp
  10016a:	83 c0 54             	add    $0x54,%eax
  10016d:	83 fd 10             	cmp    $0x10,%ebp
  100170:	75 94                	jne    100106 <interrupt+0x4b>
  100172:	83 cd ff             	or     $0xffffffff,%ebp
		run(current);
  ...
  100175:	89 6b 20             	mov    %ebp,0x20(%ebx)
		run(current);
  100178:	83 ec 0c             	sub    $0xc,%esp
  10017b:	ff 35 30 9f 10 00    	pushl  0x109f30
  100181:	e8 c6 02 00 00       	call   10044c <run>
	case INT_SYS_YIELD:
		...
		schedule();
  100186:	e8 01 ff ff ff       	call   10008c <schedule>
		// non-runnable.
  ...
  10018b:	a1 30 9f 10 00       	mov    0x109f30,%eax
		current->p_exit_status = current->p_registers.reg_eax;
		if (current->p_wait_pid != 0)
  100190:	8b 50 50             	mov    0x50(%eax),%edx
		// The process stored its exit status in the %eax register
		// before calling the system call.  The %eax REGISTER has
		// changed by now, but we can read the APPLICATION's setting
		// for this register out of 'current->p_registers'.
		current->p_state = P_ZOMBIE;
		current->p_exit_status = current->p_registers.reg_eax;
  100193:	8b 48 20             	mov    0x20(%eax),%ecx
		// non-runnable.
		// The process stored its exit status in the %eax register
		// before calling the system call.  The %eax REGISTER has
		// changed by now, but we can read the APPLICATION's setting
		// for this register out of 'current->p_registers'.
		current->p_state = P_ZOMBIE;
  100196:	c7 40 48 03 00 00 00 	movl   $0x3,0x48(%eax)
		current->p_exit_status = current->p_registers.reg_eax;
		if (current->p_wait_pid != 0)
  10019d:	85 d2                	test   %edx,%edx
		// The process stored its exit status in the %eax register
		// before calling the system call.  The %eax REGISTER has
		// changed by now, but we can read the APPLICATION's setting
		// for this register out of 'current->p_registers'.
		current->p_state = P_ZOMBIE;
		current->p_exit_status = current->p_registers.reg_eax;
  10019f:	89 48 4c             	mov    %ecx,0x4c(%eax)
		if (current->p_wait_pid != 0)
  1001a2:	74 1a                	je     1001be <interrupt+0x103>
		{
			proc_array[current->p_wait_pid].p_state = P_RUNNABLE;
  1001a4:	6b d2 54             	imul   $0x54,%edx,%edx
			proc_array[current->p_wait_pid].p_registers.reg_eax = current->p_exit_status;
			current->p_wait_pid = 0;
  1001a7:	c7 40 50 00 00 00 00 	movl   $0x0,0x50(%eax)
		// for this register out of 'current->p_registers'.
		current->p_state = P_ZOMBIE;
		current->p_exit_status = current->p_registers.reg_eax;
		if (current->p_wait_pid != 0)
		{
			proc_array[current->p_wait_pid].p_state = P_RUNNABLE;
  1001ae:	c7 82 d0 91 10 00 01 	movl   $0x1,0x1091d0(%edx)
  1001b5:	00 00 00 
			proc_array[current->p_wait_pid].p_registers.reg_eax = current->p_exit_status;
  1001b8:	89 8a a8 91 10 00    	mov    %ecx,0x1091a8(%edx)
			current->p_wait_pid = 0;
		}
		current->p_state = P_EMPTY;
  1001be:	c7 40 48 00 00 00 00 	movl   $0x0,0x48(%eax)
		schedule();
  ...
  1001ca:	a1 30 9f 10 00       	mov    0x109f30,%eax
  1001cf:	8b 50 20             	mov    0x20(%eax),%edx
		if (p <= 0 || p >= NPROCS || p == current->p_pid
  1001d2:	8d 4a ff             	lea    -0x1(%edx),%ecx
  1001d5:	83 f9 0e             	cmp    $0xe,%ecx
  1001d8:	77 14                	ja     1001ee <interrupt+0x133>
  1001da:	3b 10                	cmp    (%eax),%edx
  1001dc:	74 10                	je     1001ee <interrupt+0x133>
		    || proc_array[p].p_state == P_EMPTY)
  1001de:	6b d2 54             	imul   $0x54,%edx,%edx
  1001e1:	8d 8a 90 91 10 00    	lea    0x109190(%edx),%ecx
  1001e7:	8b 59 40             	mov    0x40(%ecx),%ebx
		// (In the Unix operating system, only process P's parent
		// can call sys_wait(P).  In MiniprocOS, we allow ANY
		// process to call sys_wait(P).)
		...
		pid_t p = current->p_registers.reg_eax;
		if (p <= 0 || p >= NPROCS || p == current->p_pid
  1001ea:	85 db                	test   %ebx,%ebx
  1001ec:	75 09                	jne    1001f7 <interrupt+0x13c>
		    || proc_array[p].p_state == P_EMPTY)
			current->p_registers.reg_eax = -1;
  1001ee:	c7 40 20 ff ff ff ff 	movl   $0xffffffff,0x20(%eax)
		// (In the Unix operating system, only process P's parent
		// can call sys_wait(P).  In MiniprocOS, we allow ANY
		// process to call sys_wait(P).)
		...
		pid_t p = current->p_registers.reg_eax;
		if (p <= 0 || p >= NPROCS || p == current->p_pid
  1001f5:	eb 2d                	jmp    100224 <interrupt+0x169>
		    || proc_array[p].p_state == P_EMPTY)
			current->p_registers.reg_eax = -1;
		else if (proc_array[p].p_state == P_ZOMBIE)
  1001f7:	83 fb 03             	cmp    $0x3,%ebx
  1001fa:	75 12                	jne    10020e <interrupt+0x153>
		{
			current->p_registers.reg_eax = proc_array[p].p_exit_status;
  1001fc:	8b 92 d4 91 10 00    	mov    0x1091d4(%edx),%edx
			proc_array[p].p_state = P_EMPTY;
  100202:	c7 41 40 00 00 00 00 	movl   $0x0,0x40(%ecx)
		if (p <= 0 || p >= NPROCS || p == current->p_pid
		    || proc_array[p].p_state == P_EMPTY)
			current->p_registers.reg_eax = -1;
		else if (proc_array[p].p_state == P_ZOMBIE)
		{
			current->p_registers.reg_eax = proc_array[p].p_exit_status;
  100209:	89 50 20             	mov    %edx,0x20(%eax)
  10020c:	eb 16                	jmp    100224 <interrupt+0x169>
			proc_array[p].p_state = P_EMPTY;
		}
		else
		{
			current->p_registers.reg_eax = WAIT_TRYAGAIN;
  10020e:	c7 40 20 fe ff ff ff 	movl   $0xfffffffe,0x20(%eax)
			current->p_state = P_BLOCKED;
  100215:	c7 40 48 02 00 00 00 	movl   $0x2,0x48(%eax)
			proc_array[p].p_wait_pid = current->p_pid ;
  10021c:	8b 00                	mov    (%eax),%eax
  10021e:	89 82 d8 91 10 00    	mov    %eax,0x1091d8(%edx)
		}
		schedule();
  100224:	e8 63 fe ff ff       	call   10008c <schedule>
  100229:	eb fe                	jmp    100229 <interrupt+0x16e>
...
start(void)
{
  ...
  10022f:	68 40 05 00 00       	push   $0x540
  100234:	6a 00                	push   $0x0
  100236:	68 88 91 10 00       	push   $0x109188
  10023b:	e8 48 03 00 00       	call   100588 <memset>
  100240:	b8 88 91 10 00       	mov    $0x109188,%eax
  100245:	31 d2                	xor    %edx,%edx
  100247:	83 c4 10             	add    $0x10,%esp
	for (i = 0; i < NPROCS; i++) {
		proc_array[i].p_pid = i;
  10024a:	89 10                	mov    %edx,(%eax)
	int whichprocess;
	pid_t i;
  ...
  10024d:	c7 40 48 00 00 00 00 	movl   $0x0,0x48(%eax)
		proc_array[i].p_wait_pid = 0;
  100254:	c7 40 50 00 00 00 00 	movl   $0x0,0x50(%eax)
	int whichprocess;
	pid_t i;
	...
	memset(proc_array, 0, sizeof(proc_array));
	for (i = 0; i < NPROCS; i++) {
  10025b:	83 c0 54             	add    $0x54,%eax
  10025e:	83 fa 10             	cmp    $0x10,%edx
  100261:	75 e7                	jne    10024a <start+0x1f>
		proc_array[i].p_state = P_EMPTY;
		proc_array[i].p_wait_pid = 0;
	}
  ...
  100263:	c7 05 30 9f 10 00 dc 	movl   $0x1091dc,0x109f30
  10026a:	91 10 00 
	...
	// first process.
	segments_init();
  10026d:	e8 72 00 00 00       	call   1002e4 <segments_init>
	special_registers_init(current);
  100272:	83 ec 0c             	sub    $0xc,%esp
  100275:	ff 35 30 9f 10 00    	pushl  0x109f30
  10027b:	e8 e3 01 00 00       	call   100463 <special_registers_init>

	// Erase the console, and initialize the cursor-position shared
	// variable to point to its upper left.
	console_clear();
  100280:	e8 2e 01 00 00       	call   1003b3 <console_clear>

	// Figure out which program to run.
	cursorpos = console_printf(cursorpos, 0x0700, "Type '1' to run mpos-app, or '2' to run mpos-app2.");
  100285:	83 c4 0c             	add    $0xc,%esp
  100288:	68 18 0a 10 00       	push   $0x100a18
  10028d:	68 00 07 00 00       	push   $0x700
  100292:	ff 35 00 00 06 00    	pushl  0x60000
  100298:	e8 4d 07 00 00       	call   1009ea <console_printf>
  10029d:	83 c4 10             	add    $0x10,%esp
  1002a0:	a3 00 00 06 00       	mov    %eax,0x60000
	do {
		whichprocess = console_read_digit();
  1002a5:	e8 4c 01 00 00       	call   1003f6 <console_read_digit>
	} while (whichprocess != 1 && whichprocess != 2);
  1002aa:	8d 58 ff             	lea    -0x1(%eax),%ebx
  1002ad:	83 fb 01             	cmp    $0x1,%ebx
  1002b0:	77 f3                	ja     1002a5 <start+0x7a>
	console_clear();
  1002b2:	e8 fc 00 00 00       	call   1003b3 <console_clear>
  ...
  1002b7:	a1 30 9f 10 00       	mov    0x109f30,%eax
  1002bc:	83 c0 34             	add    $0x34,%eax
  1002bf:	52                   	push   %edx
  1002c0:	52                   	push   %edx
  1002c1:	50                   	push   %eax
  1002c2:	53                   	push   %ebx
  1002c3:	e8 d0 01 00 00       	call   100498 <program_loader>

	// Set the main process's stack pointer, ESP.
	current->p_registers.reg_esp = PROC1_STACK_ADDR + PROC_STACK_SIZE;
  1002c8:	a1 30 9f 10 00       	mov    0x109f30,%eax
  1002cd:	c7 40 40 00 00 2c 00 	movl   $0x2c0000,0x40(%eax)

	// Mark the process as runnable!
	current->p_state = P_RUNNABLE;
  1002d4:	c7 40 48 01 00 00 00 	movl   $0x1,0x48(%eax)

	// Switch to the main process using run().
	run(current);
  1002db:	89 04 24             	mov    %eax,(%esp)
  1002de:	e8 69 01 00 00       	call   10044c <run>
  1002e3:	90                   	nop
...
segments_init(void)
{
  ...
  1002e4:	b8 c8 96 10 00       	mov    $0x1096c8,%eax
...
segments_init(void)
{
  ...
  100305:	88 15 b8 1a 10 00    	mov    %dl,0x101ab8
	...
	for (i = 0; i < sizeof(interrupt_descriptors) / sizeof(gatedescriptor_t); i++)
		SETGATE(interrupt_descriptors[i], 0,
  10030b:	ba 30 97 10 00       	mov    $0x109730,%edx
segments_init(void)
{
	int i;
...
segments_init(void)
{
  ...
  10032e:	c7 05 cc 96 10 00 00 	movl   $0x80000,0x1096cc
  100335:	00 08 00 
	kernel_task_descriptor.ts_ss0 = SEGSEL_KERN_DATA;
  100338:	66 c7 05 d0 96 10 00 	movw   $0x10,0x1096d0
  10033f:	10 00 
	...
	for (i = 0; i < sizeof(interrupt_descriptors) / sizeof(gatedescriptor_t); i++)
		SETGATE(interrupt_descriptors[i], 0,
  100341:	66 89 0c c5 30 97 10 	mov    %cx,0x109730(,%eax,8)
  100348:	00 
...
run(process_t *proc)
{
  10044c:	8b 44 24 04          	mov    0x4(%esp),%eax
	current = proc;
  100450:	a3 30 9f 10 00       	mov    %eax,0x109f30
